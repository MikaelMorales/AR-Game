...
void drawQuad(QuadGraph q, ArrayList<PVector> lines) {
  List<int[]> quads = q.findCycles();
  TwoDThreeD to3D = new TwoDThreeD(width, height);
  for (int[] quad : quads) {
    ...
    if (q.isConvex(c12, c23, c34, c41) && q.nonFlatQuad(c12, c23, c34, c41)) {
      ...
      List<PVector> temp = Arrays.asList(c12,c23,c34,c41);
      List<PVector> ordered = q.sortCorners(temp);
      PVector rotation3D = to3D.get3DRotations(ordered);
      println("rx :" + Math.toDegrees(rotation3D.x) + "°, ry : " + Math.toDegrees(rotation3D.y) + "°, rz :" + Math.toDegrees(rotation3D.z) + "°");
      q.rotX = (float)Math.toDegrees(rotation3D.x);
      q.rotZ = (float)Math.toDegrees(rotation3D.y);
    }
  }
}